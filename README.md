...
- **中英对照栏**：文章视图一键开关，按需逐段翻译当前所在段落，默认关闭不耗 API
- **翻译弹窗**：原文与翻译分区显示，支持滚动查看
- **翻译缓存**：对照栏与弹窗共用一份缓存，避免重复请求、降低 API 成本
...
进入评论或文章视图后，`Esc` 或 `b` 返回列表。

### 文章阅读视图

文章正文支持「中英对照栏」：默认关闭，纯英文阅读；需要时一键开启，下半屏会自动显示**当前所在段落**的中文翻译，随光标移动而切换——相当于免去反复手动翻译。
...
| `t` | 弹窗翻译当前段（一次性） |
| `T` | 开 / 关中英对照栏 |
| `PageUp` / `PageDown` | 滚动对照栏译文 |
| `Esc` / `b` | 返回列表 |

对照栏按需逐段翻译，并与弹窗翻译**共用同一份缓存**：关闭时不产生任何 API 调用，打开后也只翻你实际读到的段落。
...
当前共有 120+ 个单元测试，覆盖范围包括：

- 文本测量与按显示宽度换行（CJK 双宽、超长单词、URL 高亮）
- 文章抽取与噪音/署名过滤、段落切分与显示行的起止定位
- 中英对照栏的开关、按需翻译、与弹窗共用缓存、错误不入缓存
- 配置字段映射、翻译缓存读写、LLM 与 Algolia 接口的错误处理
- 首页刷新失败时不清空已有内容
